				...
				<td align='right'>Client:</td>
					<td>
						<select name='id_client'>						
							...
							?>						
							<option value = "<?php echo $id_client; ?>"><?php echo $row2[0];?></option>
							<?php
					...
					<td>
						<select name='id_product'>
							<?php				
								...
								$row2=mysqli_fetch_row($result);								
							?>	
							<option value="<?php echo $id_product; ?>"><?php echo "Name --> ".$row2[1]  ." | Price --> ".$row2[2] ." | Available --> ".$row2[3]?></option>
							<?php
								$sql2 = "SELECT id_product, name, price, quantity FROM products";
								$result2 = mysqli_query($conn, $sql2);
								while ($product = mysqli_fetch_row($result2)):
								?>
							<option value="<?php echo $product[0] ?>"><?php echo "Name --> ".$product[1]  ." | Price --> ".$product[2] ." | Available --> ".$product[3]?></option>
							<?php endwhile; ?>
						</select>
					</td>
			</tr>
